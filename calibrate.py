...
def calc_transfer_matrix(img_path, camera_matrix, dist_coeff, criteria, objp):
    ...
    if ret == True:
        ...
        cv.Rodrigues(rvecs, rotation) # https://docs.opencv.org/trunk/d9/d0c/group__calib3d.html#ga61585db663d9da06b68e70cfbf6a1eac
...
def get_calibration_data(path_intrinsics, img_path):
    ...
    objp[:, :2] = np.mgrid[0:number_of_squars, 0:4].T.reshape(-1, 2)*width # the size of a square is 7cm ; it is in meters

    trans_world2color, trans_depth2color, camera_matrix, dist_coeff = get_transfer_matrix(path_intrinsics, img_path, criteria, objp)
